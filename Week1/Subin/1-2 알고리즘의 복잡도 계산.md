...
-프로그램 특성과는 무관하게 프로그램의 문법적인 혹은 논리적인 단위
...
-정의: f(n) = 오메가(g(n))이다. →모든 n,n>n_0에 대해 f(n)≥cg(n)을 만족하는 두 양의 상수 c와 n_0가 존재하기만 하면!!
...
-정의: f(n)=세타(g(n)) → 모든 n,n>n_0에 대해 c_1g(n)≤f(n)≤c_2g(n)을 만족하는 세 양의 상수 c_1, c_2와 n_0가 존재하기만 하면!!
