class Solution {
    ...
    public boolean checkAvailableMoving(int a, int b, int []g, int[]s, int []w, int[]t, long time){
        ...
        for(int i = 0 ;i<totalCity;i++){
            ...
            totalGold +=Math.min(curGold,curMaxWeight);//현재 도시에서 최대로 운반할 수 있는 금의 양
            totalSilver +=Math.min(curSilver,curMaxWeight);//현재 도시에서 최대로 운반할 수 있는 은의 양
            totalWeight +=Math.min(curGold+curSilver,curMaxWeight);//현재 도시에서 최대로 운반할 수 있는 전체 자원의 양
        }
    ...
    public long solution(int a, int b, int[] g, int[] s, int[] w, int[] t) {
        long start = 1;
        long end = maxCyCleTime * maxWeight;//걸릴 수 있는 최악의 시간
        long answer = end;
